import { ReactNode, Suspense, lazy, createContext, useContext, useEffect, useRef, useState, useCallback } from "react";
...
import { useUITheme, useUIDesign, type UITheme } from "@/lib/uiTheme";
import { Button } from "@/components/ui/button";
import { Loader, PageLoader } from "@/components/ui/loader";
import { Dialog, DialogContent, DialogHeader, DialogTitle } from "@/components/ui/dialog";
import { NAV } from "./nav";
import { searchEverything, fetchNotifications, type SearchResult, type Notif } from "./adminQueries";
...
// SAP design: real SAP UI5 Web Components — a separate chunk, fetched only for that design
const SapShell = lazy(() => import("@/sap/SapShell"));
...
export const AdminShell = () => {
  ...
  // SAP: the UI5 ShellBar + side navigation replace every other chrome (the page, AI assistant,
  // welcome card and what's-new popup are shared)
  if (sap) {
    return (
      <>
        <Suspense fallback={<PageLoader />}>
          <SapShell title={title} navStyle={navStyle}>{page}</SapShell>
        </Suspense>
        <AIAssistant />
        <WhatsNewDialog />
        <WelcomeSplash />
