...
interface Doc {
  doc_type: string; doc_number: string; doc_date: string; due_date?: string;
...
export const generateDocPDF = async (doc: Doc, lines: Line[], party: Party, s: Settings) => {
  ...
  const logoData = await loadLogo(logoSrc);
  if (logoData) {
  ...
  pdf.text(titleFor(doc.doc_type), W - 12, 12, { align: "right" });
  pdf.setFont("helvetica", "normal"); pdf.setFontSize(8);
  ...
  const rightHeight = 28 + Math.max(0, docNumberLines.length - 1) * lineGap + Math.max(0, placeSupplyLines.length - 1) * lineGap + Math.max(0, dueDateLines.length - 1) * lineGap;
  ...
  pdf.setFont("helvetica", "normal"); pdf.setFontSize(8);
  pdf.text("Document #", rightX, y + 5);
  pdf.text(docNumberLines, rightX, y + 9);
  pdf.setFont("helvetica", "normal"); pdf.text("Date", rightX, y + 14 + Math.max(0, docNumberLines.length - 1) * lineGap);
  pdf.setFont("helvetica", "normal"); pdf.text(dateLines, rightX, y + 18 + Math.max(0, docNumberLines.length - 1) * lineGap);
  if (doc.due_date) {
    pdf.setFont("helvetica", "normal"); pdf.text("Due Date", rightX + 50, y + 14 + Math.max(0, docNumberLines.length - 1) * lineGap);
    pdf.setFont("helvetica", "normal"); pdf.text(dueDateLines, rightX + 50, y + 18 + Math.max(0, docNumberLines.length - 1) * lineGap);
  }
  const placeY = y + 23 + Math.max(0, docNumberLines.length - 1) * lineGap + Math.max(0, dueDateLines.length - 1) * lineGap;
  pdf.setFont("helvetica", "normal"); pdf.text("Place of Supply", rightX, placeY);
  pdf.setFont("helvetica", "normal"); pdf.text(placeSupplyLines, rightX, placeY + 4);
