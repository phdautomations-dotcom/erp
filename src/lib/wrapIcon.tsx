...
// Renders the Lucide icon everywhere except the iOS theme, where it becomes the
// matching SF-Symbols-style glyph. Sizing works the same (className / size props).
export function wrapIcon(name: string, Base: LucideIcon): LucideIcon {
  const Icon = forwardRef<SVGSVGElement, LucideProps>((props, ref) => {
    ...
    const d = theme === "ios" ? getIosGlyph(name) : undefined;
