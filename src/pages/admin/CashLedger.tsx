...
export default function CashLedger() {
  ...
  const [form, setForm] = useState<any>({ entry_date: new Date().toISOString().slice(0, 10), type: "in", amount: 0, description: "" });
  ...
  const save = async () => {
    ...
    setForm({ entry_date: new Date().toISOString().slice(0, 10), type: "in", amount: 0, description: "" });
    load();
  };
    ...
    <AdminLayout title="Cash Ledger">
      <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogTrigger asChild><Button className="rounded-full bg-foreground text-background hover:bg-foreground/90"><Plus className="h-4 w-4 mr-1" /> New Entry</Button></DialogTrigger>
          <DialogContent>
            <DialogHeader><DialogTitle>New Cash Entry</DialogTitle></DialogHeader>
        ...
        <div className="overflow-x-auto">
          <table className="w-full min-w-[720px] text-sm">
            ...
            <tbody className="divide-y divide-border/50">
              ...
              {rows.length === 0 && <tr><td colSpan={7} className="p-12 text-center text-muted-foreground font-medium">No cash entries yet.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
