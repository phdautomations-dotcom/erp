-- Prefix format changed client-side from 'PHDINV-2627-0220' to
-- 'PHDINV26-27-0220' (hyphenated year, one fewer separator, so it still
-- fits GST's 16-char limit). The duplicate-safety check inside this
-- function pattern-matches the compact year form against saved documents
-- to auto-heal the counter — that pattern needs to follow the same
-- change, or it stops recognizing documents saved in the new format.
--
-- Nothing else changes: same row-locked reservation, same counting logic.
